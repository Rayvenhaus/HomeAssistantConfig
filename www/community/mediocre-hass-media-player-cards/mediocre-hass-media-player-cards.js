...
`;function Qa({onTap:e,onLongPress:t,onDoubleTap:r}){const n=ue(null),o=ue(0),i=ue(null),a=ue(null),[s,d]=j(!1),[c,f]=j(!1),[l,_]=j({x:0,y:0}),p=k(()=>{i.current&&clearTimeout(i.current),a.current&&clearTimeout(a.current),f(!1),n.current=null,o.current=0},[]),m=()=>c?u(Ja,{style:{left:`${l.x}px`,top:`${l.y}px`}}):null,h=k(()=>{const P=Date.now();return!!(n.current&&P-n.current>=500)},[]),v=k((P,T)=>{_({x:P,y:T})},[]),y=k((P,T)=>{n.current=Date.now(),v(P,T),a.current=setTimeout(()=>{f(!0)},500)},[v]),b=k((P,T)=>{c&&v(P,T)},[c,v]),S=k(()=>{i.current&&clearTimeout(i.current),a.current&&(clearTimeout(a.current),f(!1)),o.current+=1,i.current=setTimeout(async()=>{d(!0);try{o.current>1?await(r==null?void 0:r()):h()?await(t==null?void 0:t()):await(e==null?void 0:e())}catch(P){console.error("Error in button callback:",P)}d(!1),n.current=null,o.current=0},250)},[h,r,t,e]),E=k(P=>{y(P.clientX,P.clientY)},[y]),g=k(P=>{b(P.clientX,P.clientY)},[b]),w=k(P=>{if(P.touches.length>0){const T=P.touches[0];y(T.clientX,T.clientY)}},[y]),C=k(P=>{if(P.touches.length>0){const T=P.touches[0];b(T.clientX,T.clientY)}},[b]);return G(()=>({...Za?{onTouchStart:w,onTouchMove:C,onTouchEnd:S,onTouchCancel:p,loading:s}:{onMouseDown:E,onMouseMove:g,onMouseUp:S,onMouseOut:p,loading:s},renderLongPressIndicator:m}),[E,g,S,p,w,C,m])}function dr(e){const{attributes:{shuffle:t,repeat:r,source:n,supported_features:o}}=e,i=(o|16)===o,a=(o|32)===o,s=t!==void 0&&!["optical","aux"].includes(n==null?void 0:n.toLowerCase()),d=r!==void 0&&!["optical","aux"].includes(n==null?void 0:n.toLowerCase()),c=(o&4096)===4096||(o&16384)===16384;return G(()=>({supportPreviousTrack:i,supportNextTrack:a,supportsShuffle:s,supportsRepeat:d,supportsTogglePlayPause:c}),[i,a,s,d,c])}const es=x.div`
  display: flex;
  flex-direction: row;
  align-items: center;
